...
// Initialize constants for extrapolation types.
...
// Constructor: Loads and precomputes channel data.
Channel::Channel(Tokenizer* fileToken)
{
    load(fileToken); // Load keyframe data from the file.
    precompute(); // Precompute tangents and cubic coefficients for the keyframes.

	// If keyframes does not exist
	if (keyframes.size() == 0)
	{
		std::cout << "No keyframe in channel" << std::endl;
	}

	// If keyframes exist, set the channel's time span and calculate period and offset.
	else
	{
...
// Destructor: Deletes all dynamically allocated keyframes.
Channel::~Channel()
{
...
// Loads channel data from tokenized input, creating keyframes and setting extrapolation rules.
...
// Precomputes tangents and cubic coefficients for interpolation between keyframes.
...
// Evaluates the channel's value at a given time using keyframe interpolation and extrapolation rules.
// evaluate the channel's value at time 't' based on keyframes and extrapolation.
